...
Enemy::Enemy(SDL_Texture* tex, int tile_size, int val_x, int val_y) {
...
bool Enemy::Touch(int player_x, int player_y) {
    if(player_x >= m_x and player_y >= m_y and player_x <= m_x + m_enemy_size_w and player_y <= m_y + m_enemy_size_h) return true;
    return false;
}
...
int Enemy::GetSizeW() const {
    return m_enemy_size_w;
}

int Enemy::GetSizeH() const {
